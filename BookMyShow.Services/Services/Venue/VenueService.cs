...
namespace BookMyShowAPI.Services
{
    ...
    public class VenueService : IVenueService
    {
        ...
        {
            var query = this.database.Query<Services.Models.Venue>("SELECT * FROM Venue WHERE Id = @0", venueId).Select(s => s.CreateVenueCoreModel());
            return query.FirstOrDefault();
        }

        public IEnumerable<Venue> GetVenuesForShow(int showId)
        {
            var query = this.database.Query<Services.Models.Venue>("SELECT * FROM Venue WHERE ShowId = @0", showId).Select(s => s.CreateVenueCoreModel()); ;
            return query;
        }
