...
namespace BookMyShowAPI.Services
{
    public class ReservationService : IReservationsService
    {
        ...
        {
            return this.database.Query<Services.Models.Reservation>("SELECT * FROM Reservation").Select(s => { return s.CreateReservationCoreModel(); });
        }
        ...
        public IEnumerable<UserReservation> GetUserReservations(string userId)
        {
            ...
            var reservationAndVenue = this.database.Query<Services.Models.Reservation, Services.Models.Venue, Services.Models.ReservationAndVenue>((a, u) => { return new Services.Models.ReservationAndVenue { Reservation = a, Venue = u }; }, reservationAndVenueJoinSql);

            var showAndVenueJoinSql = PetaPoco.Sql.Builder.Append("SELECT Show.*,Venue.* FROM Show INNER JOIN Venue ON Show.Id = Venue.ShowId");

            var showAndVenueJoin = this.database.Query<Services.Models.Show, Services.Models.Venue, Services.Models.ShowAndVenue>((a, u) => { return new Services.Models.ShowAndVenue { Show = a, Venue = u }; }, showAndVenueJoinSql);

            //format the join result 
            var venueAndReservationList = (from p in reservationAndVenue group p.Reservation by p.Venue.Id into g select (venueDetails: g.Key, reservation: g.ToList())).ToList();
            ...
            foreach (var i in venueAndReservationList)
            {
                finalResult.Add(new UserReservation
                {
                    ...
                    Reservations = i.reservation.Select(e=>e.CreateReservationCoreModel())
                });
            }
        ...
        {
            return this.database.Query<Services.Models.Reservation>("SELECT * FROM Reservation WHERE Id = @0", reservationId).Select(s => { return s.CreateReservationCoreModel(); }).FirstOrDefault();
        }

        public List<Reservation> GetReservationsForVenue(int venueId)
        {
            return this.database.Query<Services.Models.Reservation>("SELECT * FROM Reservation WHERE VenueId =@0", venueId).Select(s => { return s.CreateReservationCoreModel(); }).ToList();

        }

        public void CancelReservation(Reservation reservation)
        {
            var getReservation = this.database.Query<Services.Models.Reservation>("SELECT * FROM Reservation WHERE Id = @0", reservation.Id).FirstOrDefault();
