...
namespace BookMyShowAPI.Services.ShowService
{
    public class ShowService : IShowService
    {
        ...
        public List<Show> GetAllShows()
        {
            ...
            return this.database.Query<Models.DataModels.Show>("SELECT * FROM Show s WHERE EXISTS( SELECT * FROM Venue v WHERE s.Id = v.ShowId AND v.ShowDateTime > GETDATE()) ORDER BY s.Id DESC").Select(s => {return s.CreateShowCoreModel(); }).ToList();
        }

        public Show GetShow(int showId)
        {
            var query = this.database.Query<Models.DataModels.Show>("SELECT * FROM Show WHERE Id = @0", showId).Select(s => s.CreateShowCoreModel()); ; 
